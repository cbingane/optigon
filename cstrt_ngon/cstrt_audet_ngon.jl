...
if mod(log2(n),1)==0 && n>=16
    ...
    a[Int(n-1)] = a[Int(n/2-1)]-sin(2*z); a[1] = -a[n-1];
    b[Int(n-1)] = b[Int(n/2-1)]-cos(2*z); b[1] = b[n-1];
    a[Int(n-2)] = a[Int(n/2-1)]-sin(3*z); a[2] = -a[n-2];
    b[Int(n-2)] = b[Int(n/2-1)]-cos(3*z); b[2] = b[n-2];
    for i = 1:Int(n/8-1)

...
if mod(n,2)==0 && n>=10
    ...
    syms u w1 d1
    v = (pi/2-u-2*w1)/(n/2-3);
    d2 = asin(sin(u)-sin(u+w1+d1)+sin(u+2*w1)+sin(u+2*w1+3*v/2)/(2*cos(v/2)))-u-2*w1-v;
    a0 = u; a1 = w1+d1; a2 = w1-d1; a3 = v+d2; a4 = v-d2;
    ...
    A = A1+A2+A3+A4+A5;
    fu = diff(A,u); fw1 = diff(A,w1); fd1 = diff(A,d1);
    sol = vpasolve([fu==0;fw1==0;fd1==0],[u;w1;d1],[pi/(2*n-2), pi/n; pi/n, 2*pi/n; 0, pi/n]);
    u0 = double(sol.u); w10 = double(sol.w1); d10 = double(sol.d1);
    v0 = (pi/2-u0-2*w10)/(n/2-3);
